...
The \java{Math.ceil} method returns the {\em ceiling} of a number: the smallest value that is greater than or equal to the argument that is equal to an integer value.  Thus, \java{Math.ceil(3.0)} is 3.0; \java{Math.ceil(3.001)} is 4.0, as is \java{Math.ceil(3.999)}.
