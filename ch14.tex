...
Write a subclass \java{PreferredCustomer} that extends the \java{Customer} class from the preceding exercise. The \java{PrefCust} class should have two data fields: {\em average annual purchase amount} (average dollar amount purchased per year) and {\em years} (number of years they have been a customer). These are both private variables.
...
\begin{itemize}
\item Bronze: $\geq$ 1 year and average purchase amount $\geq$ \$5000 per year---5\% discount
\item Silver: $\geq$ 2 years and average purchase amount $\geq$ \$10000 per year---7.5\% discount
\item Gold: $\geq$ 3 years and average purchase amount $\geq$ \$15000 per year---10\% discount
\end{itemize}
...
Write the \java{PrefCust} class with all its data fields. Please write all the getter and setter methods.  Write a method named \java{getDiscount} that uses the average annual purchase amount and years as a customer to return the discount percent (as a percentage).
\end{exercise}
...
\item Implement getters and setters: \java{getNumber()}, \java{getBalance()}, and \newline \java{setBalance(double newBalance)}. There is no \java{setNumber} method---once an account is created, its account number cannot change.

\item Implement these methods: \java{void deposit(double amount)} and \newline \java{void withdraw(double amount)}. For both these methods, if the amount is less than zero, the account balance remains untouched. For the \java{withdraw} method, if the amount is greater than the balance, it remains untouched. {\em These methods do not print anything.}
